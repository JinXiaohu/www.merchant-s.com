
<style type="text/css">
.ul_feature{
  padding-left: 20px;
}
.ul_feature li{
  padding-bottom: 5px;
}
</style>


<div id="main">
  <div class="sidebar sidebar-lg">
    <ul>
      <li class="level1 grid-12"><a href="<?php echo $base_path;?>terminal">クレジットカード端末</a></li>
      <li class="level2 grid-6"><a href="<?php echo $base_path;?>terminal/s80">PAX S80</a></li>
      <li class="level2 grid-6 active"><a href="<?php echo $base_path;?>terminal/s90">PAX S90</a></li>
         
      <div class="li_sep hidden"></div>   

      <li class="level1 grid-12 hidden"><a href="<?php echo $base_path;?>terminal/business_center">センター事業</a></li>
    </ul>
  </div>

  <div class="rightbar">
    <div class="mb_separator div_hidden"></div>
    <h1><img src="<?php echo $cdn_path;?>images/terminal/s90_1.jpg"></h1>
    <h1>決済端末種別 - PAX S90</h1>

    <h2>スペック＆特徴</h2>
    <ul class="ul_feature">
      <li>高速通信のモバイル（無線）端末</li>
      <li>端末1台でクレジットカード読み取り溝、ピンパッド、プリンターを装備し<br />
      1台3役のハイスペック機能を搭載</li>
      <li>国内大手携帯電話会社の通信網を使用しハイスピード決済を実現</li>
    </ul>

    <h2>利点</h2>
    <ul class="ul_feature">
      <li>配線不要で通信環境に関わらずカード決済が可能に</li>
      <li>外付けのピンパッドやプリンターが不要で持ち運びも便利</li>
      <li>最短数秒でカード決済が可能になりお客様のストレス軽減</li>
    </ul>

    <h2>こんな方におすすめ</h2>
    <ul class="ul_feature">
      <li>飲食店でもお客様の目の前でテーブル決済が可能に</li>
      <li>屋外でのイベントや催事でもカード決済が可能に</li>
      <li>お客様のお宅へ伺い納品する際にカード決済することが可能に</li>
    </ul>


  </div>
</div>


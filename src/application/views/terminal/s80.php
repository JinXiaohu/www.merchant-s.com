
<style type="text/css">
.ul_feature{
  padding-left: 20px;
}
.ul_feature li{
  padding-bottom: 5px;
}
</style>


<div id="main">
  <div class="sidebar sidebar-lg">
    <ul>
      <li class="level1 grid-12"><a href="<?php echo $base_path;?>terminal">クレジットカード端末</a></li>
      <li class="level2 grid-6 active"><a href="<?php echo $base_path;?>terminal/s80">PAX S80</a></li>
      <li class="level2 grid-6"><a href="<?php echo $base_path;?>terminal/s90">PAX S90</a></li>
         
      <div class="li_sep hidden"></div>   

      <li class="level1 grid-12 hidden"><a href="<?php echo $base_path;?>terminal/business_center">センター事業</a></li>
    </ul>
  </div>


  <div class="rightbar">
    <div class="mb_separator div_hidden"></div>
    <h1><img src="<?php echo $cdn_path;?>images/terminal/s80_1.jpg"></h1>
    <h1>決済端末種別 - PAX S80</h1>

    <h2>スペック＆特徴</h2>
    <ul class="ul_feature">
      <li>本体にピンパッド機能を搭載</li>
      <li>インターネット回線（有線）にも対応可能</li>
      <li>店舗での回線変更にも対応可能なアナログ/LAN回線共用タイプ(準備中）</li>
    </ul>

    <h2>利点</h2>
    <ul class="ul_feature">
      <li>外付けのPINパッドが不要なため、省スペースを実現</li>
      <li>数秒でカード決済が可能になり、お客様のストレス軽減（準備中）</li>
      <li>既設の電話回線と共用可能（余計な設備投資は不要）</li>
    </ul>

    <h2>こんな方におすすめ</h2>
    <ul class="ul_feature">
      <li>レジ回りのスペースに余裕がなく端末本体とピンパッドの2台は置きたくない</li>
      <li>決済件数が多く月々の電話料金が負担： LAN（準備中）</li>
      <li>店舗にインターネット環境がない/有線ルーターまでの距離が遠い</li>
    </ul>


  </div>
</div>

